...
module "bastion_vault" {
  ...
  node_name             = var.bastion_vault_config.node_name
  vm_id                 = var.bastion_vault_config.vm_id
  hostname              = "bastion"
  datastore_id          = var.lxc_datastore_id
  template_datastore_id = var.lxc_template_datastore_id
  bridge                = var.lxc_bridge
  ipv4_address          = var.bastion_vault_config.ipv4_address
  ...
  disk_size_gb          = var.bastion_vault_config.disk_size_gb
  memory_mb             = var.bastion_vault_config.memory_mb
  swap_mb               = 512
  cpu_cores             = var.bastion_vault_config.cpu_cores
  unprivileged          = true
  nesting               = true
  keyctl                = false
  ...
  tags                  = ["infrastructure", "secrets", "bastion-vault"]
}
